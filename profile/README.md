...
```mermaid
flowchart TD
    A[Player Input] --> B(Mobility Engine)
    B --> C(Combat Analyzer)
    C --> D{Scan Output}
    D -- Enemies --> E[Tracking Logic]
    D -- Resources --> F[Alerts Module]
    D -- Objectives --> G[Beacon Overlay]
    E --> H[Assist Injection]
    F --> I[HUD Enhancements]
    G --> I
    H --> I
